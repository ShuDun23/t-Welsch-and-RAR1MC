...
[PSNR1,PSNR2,PSNR3,PSNR4,PSNR5,PSNR6,PSNR7,PSNR8,PSNR9,PSNR10,PSNR11,PSNR12,PSNR13,PSNR14] = deal(zeros(length(sigma2),K));
[SSIM1,SSIM2,SSIM3,SSIM4,SSIM5,SSIM6,SSIM7,SSIM8,SSIM9,SSIM10,SSIM11,SSIM12,SSIM13,SSIM14] = deal(zeros(length(sigma2),K));
[RMSE1,RMSE2,RMSE3,RMSE4,RMSE5,RMSE6,RMSE7,RMSE8,RMSE9,RMSE10,RMSE11,RMSE12,RMSE13,RMSE14] = deal(zeros(length(sigma2),K));
for ii = 1:length(sigma2)
    ...
    for kk = 1:K % Monte Carlo
        ...
        % Noise
        SNR = 10;
        sigma1 = (norm(M,'fro'))^2/(m*n*10^(SNR/10));
        M_noise = M + sqrt(sigma1) * randn(m,n);

        c2 = 0.1;
        AM = sigma2(ii) * max(abs(M),[],"all");
        EL0 = round(m*n*c2);
        E = zeros(m,n);
        Ind = randperm(m*n);
        ...
        tic
        X_9 = LpRtracep(M_Omega, array_Omega, 10, p);
        toc
        t_9 = [t_9 toc];
        ...
        opts.eta = 0.05;              
        opts.lambda = 0.01*norm(data,2);            % 1 for lp 0.01
        ...
        option.U=abs(1*rand(m,rak));
        option.V=abs(1*rand(rak,n));
        option.yita=2;
...
%% Plot
figure
plot(sigma2,result_RMSE(:,1),'*-','Linewidth',1.5,'Color',"#FF00FF")
hold on
plot(sigma2,result_RMSE(:,2),'o-','Linewidth',1.5,'Color',"#D95319")
plot(sigma2,result_RMSE(:,3),'s-','Linewidth',1.5,'Color',"#EDB120")
plot(sigma2,result_RMSE(:,4),'^-','Linewidth',1.5,'Color',"#7E2F8E")
plot(sigma2,result_RMSE(:,5),'d-','Linewidth',1.5,'Color',"#77AC30")
plot(sigma2,result_RMSE(:,7),'+-','Linewidth',1.5,'Color',"#A2142F") % RMC-HUBER
plot(sigma2,result_RMSE(:,8),'x-','Linewidth',1.5,'Color',"#0072BD") % RMF-MM
plot(sigma2,result_RMSE(:,9),'v-','Linewidth',1.5,'Color',"#4DBEEE") % Splp
plot(sigma2,result_RMSE(:,12),'hexagram-','Linewidth',1.5,'Color',[0.75,0.75,0]) % GUIG-log
plot(sigma2,result_RMSE(:,14),'_-','Linewidth',1.5,'Color',[0.25,0.25,0.25]) % NCPG
plot(sigma2,result_RMSE(:,10),'.-','Linewidth',1.5,'Color',[0.75,0,0.75],'MarkerSize',8) % HOAT
plot(sigma2,result_RMSE(:,13),'pentagram-','Linewidth',1.5,'Color',[0.75,0.75,0.75]) % HQ
...
filename = sprintf('synthetic data Gaussian and outlier sigma2 %s.mat',time);
