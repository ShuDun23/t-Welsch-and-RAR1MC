function [X,Y]=Smf_lr_PALM(data,support,siz,opts)% with extrapolation with upper bound of eta 1
    k=siz.k;
    [m,n] = size(data);
    ...
    if isfield(opts,'Y');                     Y         = opts.Y;                    end
...
%     fval=@(sxp, WXM,lambda) lambda*sum(sxp)+ 1/2*sum(sum(WXM.*WXM));
    ...
    for i=1:maxit   % 主循环 
        ...
        Prod = partXY(X_m',Y,row,col,length(value_1))';
        data_err = Prod - value_1;
...
%         [X,sxp(1),ind]= str_lp(Xtemp,lambda/lipz(1),p_i(1));
        ...
        Prod = partXY(X',Y_m,row,col,length(value_1))';
        data_err = Prod - value_1;
        WXM = sparse(row,col,data_err,m,n);        
