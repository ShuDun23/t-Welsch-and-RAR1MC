...
% ip is the parameter xi in (35) for our paper 
...
t_m_n = T(find(T));
scale = 10*1.4815*median(abs(t_m_n - median(t_m_n))); % scale e in (35)的初值
...
for iter = 1 : maxiter
    ...
    T = (M_Omega - X).*Omega_array;  % X-UV
    
    t_m_n = T(find(T));
    scale =min([scale ip*1.4815*median(abs(t_m_n - median(t_m_n)))]); % e 标量
    
    sigma = ones(m,n)*diag(scale);
    ONE_1=ones(m,n);
    ONE_1(abs(T-median(t_m_n))-sigma<0)=0; % 这里为什么要减median？
    N = T.*ONE_1;
